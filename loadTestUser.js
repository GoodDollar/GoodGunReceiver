...
function makePut(user, i) {
  return new Promise(resolve => {
    // console.log(i);
    const randkey = Math.random();
      user.get(randkey).put(
      {
        i,
...
function gunAuth(gunClient, username, password) {
    return new Promise((res, rej) => {
        gunClient.auth(username, password, user => {
            if (user.err) {
                console.log('ERROR-######################################')
...
function gunCreate(gunClient, username, password) {
    return new Promise((res, rej) => {
        gunClient.create(username, password, user => {
            if (user.err) {
                console.log('ERROR-######################################')
                console.log(user.err)
                console.log('ERROR-######################################')
...
async function createAndAuthGunUser (gunClient) {
    const randomEmail = faker.internet.email()
    const randomPassword = faker.internet.password()
    await gunCreate(gunClient, randomEmail, randomPassword)
    await gunAuth(gunClient, randomEmail, randomPassword)
}
...
async function runTest(maxConcurrent, totalPuts, numClients, type = "fast") {
  ...
  for (let i = 0; i < numClients; i++) {
    ...
    let gunuser = client.user()
    if (gunuser.is) {
      logger.debug('init:', 'logging out first')
      gunuser.leave()
    }
    
    await createAndAuthGunUser(gunuser)
    
    clients.push(gunuser);
  }
  while (true) {
